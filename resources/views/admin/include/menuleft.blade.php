            ...
            @if (Auth::check())
                ...
                @foreach (\App\Department::where(['status' => 1, 'type' => 2, 'status' => 1])->whereIn('id', $depart_id)->orderBy('order', 'ASC')->get() as $key => $dep)
                    ...
                    @if ($getSubDepartment->count() > 0)
                        <li
                            class="{{ Auth::user()->role_id == 3 ? '' : 'treeview' }} {{ $dep->slug == $active ? 'active' : '' }}">
                            <a
                                href="{{ Auth::user()->role_id == 3 ? route('finance') . '/' . $dep->slug : 'javascript:void(0)' }}">
                            ...
                            <ul class="treeview-menu">
                                @foreach ($getSubDepartment as $subdep)
                                    <li {{ $subdep->slug == $subactive ? 'class=active' : '' }}><a
                                            {{ empty($subdep->link) ? '' : 'target=_blank' }}
                                            href="{{ route('adminhome') }}/{{ $subdep->slug }}"><i
                                                class="fa fa-circle-o"></i>{{ $subdep->name }}</a></li>
                                @endforeach
                            </ul>
                        </li>
                    @endif
